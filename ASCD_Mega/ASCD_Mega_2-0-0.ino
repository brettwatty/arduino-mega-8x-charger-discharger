...
typedef struct
{
  ...
  const byte moduleCount = 8;                    // Number of Modules
...
void rotaryEncoder()
{
  ...
  {
    if (cycleStateLast - rotaryEncoderDirection >= 0 && cycleStateLast - rotaryEncoderDirection < settings.moduleCount)
    {
      cycleStateLast = cycleStateLast - rotaryEncoderDirection;
    } else if (cycleStateLast - rotaryEncoderDirection == settings.moduleCount) {
      cycleStateLast = 0;
    } else if (cycleStateLast - rotaryEncoderDirection < 0) {
      cycleStateLast = settings.moduleCount - 1;
    }
...
float readVoltage(uint8_t analogPin)
{
  ...
  for (byte i = 0; i < 5; i++)
  {
    batterySampleVoltage = batterySampleVoltage + (analogRead(analogPin) * (readVcc() / 1000.0) / 1023.0);
  }
